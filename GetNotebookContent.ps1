      ...
      foreach ($p in $Path) {
        ...
        foreach ($cell in $r.cells) {
            ...
            if ($IncludeOutput) {
                ...
                if (    $cellObj.Output.where({$_ -is    [hashtable]}) -and -not                #if some are hashtables and none isn't
                        $cellObj.Output.where({$_ -isnot [hashtable]}) ){
                    if ($cellObj.Output.where({     $_.containskey('text/html')}) -and -not     #if some have text/html and none doesn't.
                        $cellObj.Output.where({-not $_.containskey('text/html')}) ) {
                        $cellObj['HtmlOutput'] =  ($cellObj.output | ForEach-Object { $_.'text/html' -join "`r`n" })  -join "<br />`n"  #Add an HTML Output field
                    }
